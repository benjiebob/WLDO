...
import meshzoo
import torch
from .obj2nmr import obj2nmr_uvmap

def from_img_to_nmr(texture_map, ft, vt, tex_size):
    uv_map = obj2nmr_uvmap(ft, vt, tex_size=tex_size)
    if len(texture_map.shape) == 3:
        uv_img = uv_img.permute(1,2,0)
        nmr_texture = sample_texture(uv_map, uv_img)
    else:
        uv_img = uv_img.permute(0,2,3,1)
        nmr_texture = sample_textures(uv_map, uv_img)
    return nmr_texture
...
def sample_texture(uv_map, uv_img):
    F, T = uv_map.shape[:2]
    # H x W x 3 -> 1 x 3 x H x W
    uv_img = np.transpose(uv_img[None, :], (0, 3, 1, 2))
    # Pytorch-fy
    uv_map_t = torch.Tensor(uv_map).cuda()
    uv_img_t = torch.Tensor(uv_img).cuda()
    # make 1 x F x T x T x 2 -> 1 x F x T*T x 2
    uv_map_t = uv_map_t[None, :].view(1, F, T * T, 2)
    ...
    textures = textures.unsqueeze(4).repeat(1, 1, 1, 1, T, 1)
...
def make_symmetric(verts, faces, left_inds, right_inds, center_inds):
    ...
    # Silvia prop_left_inds = np.hstack([np.where(np.all(verts == np.array([-1, 1, 1]) * verts[ri], 1))[0] for ri in right_inds])
    #prop_left_inds = np.hstack([np.where(np.all(verts == np.array([1, -1, 1]) * verts[ri], 1))[0] for ri in right_inds])

    prop_left_inds = left_inds
    ...
    for ind, (ri, li) in enumerate(zip(right_inds, prop_left_inds)):
        # Silvia if np.any(verts[ri] != np.array([-1, 1, 1]) * verts[li]):
        #if np.any(verts[ri] != np.array([1, -1, 1]) * verts[li]):
        if np.any(np.abs(verts[ri] - np.array([1, -1, 1]) * verts[li])>1e-07):
            print('bad! %d' % ind)
...
def make_faces_symmetric(verts, faces, num_indept_verts, num_sym_verts):
    ...
    if len(indept_faces) > 0:
        new_faces = np.vstack([indept_faces, right_faces, left_faces])
    else:
        new_faces = np.vstack([right_faces, left_faces])
...
def compute_uvsampler_sphere(verts, faces, tex_size=2):
    """
...
def compute_uvsampler(verts, faces, vt, ft, tex_size=2):
    """
    For this mesh, pre-computes the UV coordinates for
    F x T x T points.
    Returns F x T x T x 2
